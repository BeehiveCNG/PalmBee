...
def launch_setup(context, *args, **kwargs):
    ...
    node_parameters.append( 
            # Launch arguments must override the YAML files values
            {
                ...
                # ==============================
                # POS TRACKING (PX4 EKF FRIENDLY)
                # ==============================
                'pos_tracking.enabled': True,

                # MUST be false, otherwise EKF rejects motion
                'pos_tracking.set_as_static': False,

                # Good for aerial vehicles
                'pos_tracking.set_gravity_as_origin': True,

                # IMPORTANT: increase tracking frequency
                'pos_tracking.pos_tracking_rate': 30.0,

                # DO NOT let ZED fight PX4 for map->odom
                'pos_tracking.publish_tf': False,
                'pos_tracking.publish_map_tf': False,

                # Still publish odometry internally
                'pos_tracking.publish_odom_tf': True,

                # ==============================
                # VELOCITY OUTPUT (CRITICAL)
                # ==============================
                'pos_tracking.enable_imu_fusion': True,
                'pos_tracking.enable_area_memory': False,
                'pos_tracking.enable_pose_smoothing': False,

                # Reduce smoothing (EKF needs motion)
                'pos_tracking.path_smoothing': False,
